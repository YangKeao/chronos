...
fn main() {
    ...
    let fake_image: String = value_t!(matches, "fake", String).unwrap();

    let mut program = program::Program::ptrace(pid).unwrap();

    let vdso_entry = program
        .select_lib(Program::generate_name_lib_selector("[vdso]"))
        .unwrap();
    let real_addr = program
        .get_func_in_lib("clock_gettime", vdso_entry)
        .unwrap();
    let handle = program.dlopen(fake_image.clone()).unwrap();
    let fake_addr = program.dlsym(handle, "fake_clock_gettime").unwrap();

    program.hard_replace_fun(real_addr, fake_addr).unwrap();

    let tv_sec_delta = unsafe {
        std::mem::transmute::<c_long, [u8; std::mem::size_of::<c_long>()]>(24833183051058)
    };
    let tv_sec_delta_ptr = program.dlsym(handle, "TV_SEC_DELTA").unwrap();
    program
        .write_slice(&tv_sec_delta, tv_sec_delta_ptr)
        .unwrap();
    ...
    program
        .write_slice(&tv_nsec_delta, tv_nsec_delta_ptr)
        .unwrap();
